...
- [ ] **`ContextSwitch::init_user_context`** added to the HAL trait ([`hal/src/context_switch.rs`](../../../../hal/src/context_switch.rs)) as an additive sibling of `init_context`, with a `# Safety` section: `unsafe fn init_user_context(&self, ctx: &mut Self::TaskContext, user_entry: usize, user_sp: usize, kernel_stack_top: *mut u8)`. The `# Safety` states the **EL0-trap stack-size contract** (`kernel_stack_top` must hold the ~272-byte trap frame + handler call tree on every `+0x400` trap — stronger than `init_context`'s cooperative ≥ 512 bytes) and the user-VA mapping/alignment obligations.
...
- [ ] **`enter_el0` trampoline** (new `#[unsafe(naked)] unsafe extern "C" fn enter_el0() -> !`): (1) reads `x19`/`x20` and installs the EL0 return state — `MSR SP_EL0, x20` → `MSR ELR_EL1, x19` → `MOV x8, #0x3C0` + `MSR SPSR_EL1, x8` (**register form** — the immediate `MSR` is PSTATE-fields-only; `0x3C0` = EL0t + `DAIF` masked); (2) **scrubs the register file** — zeroes `x0`–`x30` + `v0`–`v31` so **no kernel GPR/SIMD state leaks to EL0** (2026-05-31 review-round HIGH: the cooperative switch restores only callee-saved regs, leaving `x0`/`x1`/`x8` = kernel ptrs, `x30` = kernel code addr, caller-saved SIMD = kernel data); (3) `ERET`. Audited **UNSAFE-2026-0032**.
