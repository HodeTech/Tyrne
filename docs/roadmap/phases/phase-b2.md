...
Phase B.2 has no ADRs of its own. Every item above is a documentation correction, a CI wiring change, or a code fix that does not change any Accepted architectural decision. If, in the course of executing a track above, a fix turns out to imply an actual design decision — for example, choosing between the two `MappingFlags::DEVICE` remediation options in Track B.2-3, or deciding to add a scheduled (rather than PR-gated) perf-regression CI job in Track B.2-1 — write it up and route it through normal ADR governance (see [`.agents/skills/write-adr/SKILL.md`](../../../.agents/skills/write-adr/SKILL.md)) rather than deciding it silently inside this phase's backlog.
