...
- **[🟡 MEDIUM]** Release-profile kernel builds silently drop the migration-target sanity check.
  - Location: `bsp-qemu-virt/src/main.rs:800-808` (cross-ref `Cargo.toml:59-70`, `tools/smoke.sh:12-56`)
  - Action: Either (a) replace the `debug_assert!` with an unconditional `assert!` (the function runs exactly twice per boot — negligible cost), or (b) explicitly set `debug-assertions = true` in `[profile.release]` for the workspace, consistent with the `overflow-checks` precedent already set two lines above it in the same file.
  - Gates: Milestone D1/D7 — this guards the target of an unconditional `br` mid-boot; Pi 4's boot flow (ADR-0042) must not carry a release-mode-silent version of the same hazard.
...
- **[⚪ LOW]** GIC EOI write silently drops the CPUID subfield required for correct SGI end-of-interrupt.
  - Location: `bsp-qemu-virt/src/gic.rs:369-370 (acknowledge)` and `:378-384 (end_of_interrupt)`; `hal/src/irq_controller.rs:13 (IrqNumber)`.
  - Action: No action needed for v1 (single-core, timer-only). When the future SGI/IPI ADR is written, account for the CPUID subfield explicitly: either widen `IrqNumber` (or add a parallel raw-acknowledge type) to carry the untruncated GICC_IAR value through to `end_of_interrupt`, or scope a GIC-driver-private raw-EOI path for the SGI ID range. Leave a forward-note next to the relevant mask constant now so this is discoverable without re-deriving it later.
...
- **[⚪ LOW]** GICv2 `acknowledge()`/`end_of_interrupt()` mask away GICC_IAR's CPU-ID field (bits [12:10]), which GICv2 requires be echoed back verbatim in GICC_EOIR for SGIs.
  - Location: `bsp-qemu-virt/src/gic.rs:364-376 (acknowledge)` and `bsp-qemu-virt/src/gic.rs:378-385 (end_of_interrupt)`.
  - Action: Zero-cost today; no action required for v1. When SGI/IPI work lands, either widen `IrqNumber`/`acknowledge`'s return to preserve the raw IAR bits for the SGI case, or add a dedicated `acknowledge_sgi()` path that keeps the CPUID field through to `end_of_interrupt`.
  - Note: this is the same underlying gap as the finding immediately above it, filed separately by the review against a slightly different line range in `gic.rs`; the review's own prose cites this as gated by "ADR-0043, roadmap C4" for the eventual SGI/IPI ADR. That number now collides with Phase D's own ADR-0043 (GIC-400 register layout, per the ADR ledger below, renumbered 2026-05-22) — flag this as an ADR-numbering item to reconcile whenever Phase C's SGI/IPI ADR is actually drafted, so the two don't collide.
  - Gates: none directly in Phase D; same disposition as the finding above.
...
- **[⚪ LOW]** `high_half_alias`'s only defence against a >4 GiB kernel-image PA is a `debug_assert!`, compiled out in the release profile used for real runs.
  - Location: `bsp-qemu-virt/src/main.rs:800-808 (fn high_half_alias)`.
  - Action: Given the workspace already treats `overflow-checks` as a security property that must not be dropped for performance, apply the same reasoning: either set `debug-assertions = true` in `[profile.release]`, or promote this specific check to a real `assert!` so it survives release builds regardless of the global setting. This is directly relevant to Milestone D5's ADR-0045 (Pi 4 memory layout) — Pi 4's RAM base (`0x0000_0000`) and peripheral window differ from QEMU's, and the 4 GiB PA-mask assumption behind this check is explicitly flagged in `boot.md` as a "forward limit" that needs re-validation on real hardware.
  - Gates: Milestone D5.
...
Covers all 25 review findings + 28 polish items routed to this phase.
