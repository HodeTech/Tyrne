...
- 🟡 **MEDIUM** — W^X is unenforced by the mapping primitives: flags flow unchecked from caller through `cap_map` to `Mmu::map`, and neither `cap_map` nor the encoder rejects WRITE+EXECUTE.
  ...
  - **Action:** Enforce the invariant at the **shared, lower layer** — `Mmu::map` or a new shared `MappingFlags::validate()` (reject `WRITE && EXECUTE`, and `WRITE && EXECUTE && USER`, with `InvalidFlags`) — *not* at `cap_map`. `cap_map` is **not** the universal chokepoint: the task loader and bootstrap map pages by calling the mapper directly, bypassing `cap_map` entirely (this is exactly the direct-caller path the companion finding immediately below covers). Landing the check in the shared validation point closes the gap for every caller by construction. Keep a `WRITE && EXECUTE` rejection at `cap_map` too, but framed as **defense-in-depth** at the capability layer — so the capability path still fails closed even if a future refactor moves the lower-level check. Both are independent of, and cheaper than, the broader ADR-0034 per-section kernel-image remap work.
