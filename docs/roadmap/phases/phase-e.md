...
- 🟡 **MEDIUM** — `cap_map` performs no W^X validation; flags flow unchecked from caller to `Mmu::map`, and the encoder does not reject WRITE+EXECUTE.
  ...
  - **Action:** Add a cheap defense-in-depth guard at the `cap_map` chokepoint — the single narrowest point every mapping (kernel-internal or future syscall-driven) must pass through: reject `flags.contains(WRITE) && flags.contains(EXECUTE)` with `MmuMapError(InvalidFlags)` before calling `Mmu::map`. Independent of, and cheaper than, the broader ADR-0034 per-section kernel-image remap work; closes the gap for every future caller instead of relying on caller discipline.

- ⚪ **LOW** — `Mmu::map`/`flags_to_descriptor_bits` do not structurally reject WRITE+EXECUTE+USER — W^X is upheld only by loader-call-site discipline, not by the primitive itself (companion to the `cap_map` finding above, one layer lower in the stack).
  ...
  - **Action:** Add a second unconditional rejection in `Mmu::map` (or, better, in the shared `flags_to_descriptor_bits` / a new `MappingFlags::validate()`) for any mapping with both WRITE and EXECUTE set when USER is also set (and arguably the kernel-only case too, pending ADR-0034). Mirror the existing DEVICE+EXECUTE guard so the gap closes "by construction" for every future caller. Cheap now, before any userspace-reachable map-style syscall exists; expensive to retrofit once multiple callers depend on the permissive behavior.
