...
5. ✅ **`tyrne-user` crate** (safe wrappers) + **`userland/hello/` crate** + the `cargo build → rust-objcopy -O binary → include_bytes!` pipeline — **landed (2026-06-01, [T-027](../../analysis/tasks/phase-b/T-027-userland-build-pipeline.md) / [ADR-0039](../../decisions/0039-userland-build-pipeline.md), In Review).** `tools/build-userland.sh` (rust-objcopy from the pinned `llvm-tools-preview`, no Cargo dep) produces the git-ignored `hello.bin`; the BSP embeds it via `include_bytes!` (`build.rs` asserts it exists); `smoke.sh` + CI build userland first; UNSAFE-2026-0033 for the userspace `svc` shims. The real 117-byte image **loads dormant** (entry `0x800000`); **running** it is step 6. *(§Sub-breakdown items 1 + 3.)*
6. ✅ **Wire-up + QEMU smoke — DONE (2026-06-01, [T-028](../../analysis/tasks/phase-b/T-028-el0-userspace-wireup.md), In Review).** A true EL0 task (`userland/hello`) takes the lower-EL `+0x400` vector, the dispatcher copies `console_write` from the task's `TTBR0_EL1` (gate #1), `ERET` returns to EL0, and `task_exit` terminates it — the EL0↔EL1 round-trip B5's `+0x200` proxy could not prove, now proven at runtime (smoke: `hello from userspace` → `userspace task exited` → `all tasks complete`, zero new fault class). The wire-up (`load_image` → `task_create_from_image` → seed `DebugConsole`@`HELLO_CONSOLE_CAP` → `add_user_task`) also added a scheduler termination path (`task_exit_current`, sharing `start`'s extracted `run_dispatched` tail) — `task_exit`'s `Terminate` was dormant. **EL0-boundary security review = Approve** ([2026-06-01](../../analysis/reviews/security-reviews/2026-06-01-T-028-el0-userspace-wireup.md); 0 confirmed exploitable defects; SEC-T028-01 lifecycle forward-flagged). *(§Sub-breakdown items 2 + 4.)*
7. ✅ **Closure = the Phase B retrospective — DONE (2026-06-01).** The closure quartet filed: the [first-userspace guide](../../guides/first-userspace.md); the [B6 perf baseline](../../analysis/reviews/performance-optimization-reviews/2026-06-01-B6-closure.md) (boot-to-end band + footprint + test-count + a **same-host control** proving the EL0 task's cost is sub-floor — the per-op EL0/IPC/ctx-switch micro-measurements deferred to a focused feature-gated instrumentation task, [T-029](../../analysis/tasks/phase-b/T-029-perf-microbench.md)); the security review of the now-attacker-observable boundary ([T-028 EL0-boundary](../../analysis/reviews/security-reviews/2026-06-01-T-028-el0-userspace-wireup.md) + the [consolidated Phase-B seam review](../../analysis/reviews/security-reviews/2026-06-01-phase-b-closure.md), both **Approve**, 0 confirmed exploitable defects); and the [Phase B retrospective](../../analysis/reviews/business-reviews/2026-06-01-phase-b-closure.md). *(§Sub-breakdown items 5–7.)*
...
| [ADR-0037](../../decisions/0037-el0-entry-context.md) | EL0 entry context (B6 — userspace register file + `enter_el0` / `ERET` trampoline + per-task `SP_EL1`) | **B6 (Accepted 2026-05-31)** | drives [T-023](../../analysis/tasks/phase-b/T-023-el0-entry-context.md): `ContextSwitch::init_user_context` + BSP `enter_el0` trampoline (scrubs EL0-visible register file before `ERET` — `x0`–`x30`, `v0`–`v31`, `FPCR`/`FPSR`, `TPIDR_EL0`/`TPIDRRO_EL0`) + `Scheduler::add_user_task`. Option D2: reuse `Aarch64TaskContext`, per-task `SP_EL1` from `add_user_task`. |
| [ADR-0038](../../decisions/0038-mmu-translate-and-user-access.md) | `Mmu::translate` read-only walk + per-task user-access translation (B6 gate #1 — confused-deputy defence) | **B6 (Accepted 2026-05-31)** | drives [T-025](../../analysis/tasks/phase-b/T-025-user-access-translation.md): read-only `translate` walk query (closes ADR-0009 §Open-questions "translation walk query"); two-pass `copy_from_user` / `copy_to_user` — resolve every user page through the task's own AS, require `USER`, `FaultAddress` on any miss/non-`USER`/block; per-task `[entry_va, stack_top_va)` window first-gate. |
| [ADR-0039](../../decisions/0039-userland-build-pipeline.md) | Userland build pipeline (B6 — `userland/hello` + `tyrne-user` crate + raw-flat embed orchestration) | **B6 (Accepted 2026-05-31)** | drives [T-027](../../analysis/tasks/phase-b/T-027-userland-build-pipeline.md): `tools/build-userland.sh` → `cargo build` → `rust-objcopy -O binary` (from pinned `llvm-tools-preview`, no Cargo dep, K3-8 unfired) → git-ignored `hello.bin`; BSP embeds via `include_bytes!` (`build.rs` asserts `.bin` exists); `tyrne-user` crate provides safe `console_write`/`task_exit` `svc #0` shims (UNSAFE-2026-0033, first userspace `unsafe`). |
