...
- **Capability-aware extension point.** B5+ introduces `MemoryRegionCap` (capability-gated frame ownership). The v1 PMM is **kernel-internal at runtime** (the BSP touches it only at construction time — computing reserved ranges from linker symbols, calling `Pmm::new`, publishing the cell — and never afterwards); the future capability-system grants will sit on top of `Pmm::alloc_frame` + a reservation-tracking layer. The PMM's reservation API should be designed so the future capability layer can mark frames as "owned by capability X" without the PMM caring about the cap object.
...
- **Audit-discipline minimisation.** The PMM today should add zero new audit-log entries (subject to the §Dependency chain step 5 adjudication-deferred caveat). The frame-zeroing step is the only operation that touches kernel-owned memory the caller doesn't yet own; it requires a single `unsafe` block (`core::ptr::write_bytes` over a `*mut u8` derived from the allocated `PhysFrame`) — the operation either extends UNSAFE-2026-0001's existing umbrella (which the ADR characterises as **"kernel-static-buffer raw-pointer write to identity-mapped memory"**) via Amendment, or warrants a new entry under T-017's security-review verdict. Materialising a `&mut [u8; 4096]` slice from the raw pointer (to enable `core::slice::fill` instead) would *itself* be an `unsafe` step (`core::slice::from_raw_parts_mut`); raw `write_bytes` is the more honest expression of what the PMM is doing.
...
2. **Option B — External free-list (LIFO stack of frame indices).** Metadata: stack of 32 K × 4 bytes = 128 KiB with `u32` indices (or 64 KiB with `u16` since 32 K fits in 15 bits — but `u32` is forward-portable to BSPs with > 64 K frames such as the future Pi 4 with 1 M frames at 4 GiB). Alloc: O(1) pop. Free: O(1) push.
...
Zero new `unsafe` audit entries (subject to the §Dependency chain step 5 adjudication-deferred caveat — T-017's security-review pass may verdict that PMM frame-zeroing's safety argument is materially distant enough from PL011 MMIO blessing to warrant a new entry; either outcome is bounded). The bitmap is a `&mut [u8]` over a `[u8; PMM_BITMAP_BYTES]` static; bit-set / bit-clear is safe-Rust integer arithmetic. The frame-zeroing step is the only memory-touching operation: implemented via `core::ptr::write_bytes` on a `*mut u8` derived from the allocated `PhysFrame` (which post-MMU is identity-mapped to a kernel-readable VA per ADR-0027). The zeroing is `unsafe`, but it sits under the existing UNSAFE-2026-0001 **"kernel-static-buffer raw-pointer write to identity-mapped memory"** umbrella and gains an Amendment rather than a new entry. The PMM body itself — bitmap init, scan, set, clear, hint maintenance — is entirely safe Rust.
...
|------|-----------|--------|------------|-----------------------------------|
| 0 | Backing storage for the future `Pmm` value lives in a `StaticCell<Pmm>` whose `MaybeUninit<Pmm>` slot sits in `.bss` (zeroed by `_start`'s BSS loop, but `Pmm::new` constructs a fully-initialised value so the zero-init is precondition-only, not load-bearing) | `Pmm::new(extent: PhysFrameRange, reserved: &[PhysFrameRange]) -> Result<Self, PmmError>` performs three pre-mutation validations *before* writing any bitmap bit: (i) `extent.start` and `extent.end` are page-aligned (returns `Err(PmmError::MisalignedAddress)` if not — without this, the index math `(pa - extent.start) / 4096` could yield off-by-one indices and corrupt bitmap state); (ii) every reserved range fits inside `[extent.start, extent.end)` (returns `Err(PmmError::OutOfRange)` if a range straddles or exceeds the extent — without this, the per-range bit-set loop would write past the bitmap's `[u8; N]` bound); (iii) `reserved.len() <= R` (returns `Err(PmmError::TooManyReservedRanges)` if the caller passes more ranges than the per-BSP const-generic capacity). All three validations are **fail-fast**: on any error the constructor returns without partial mutation. After validation, walks the reserved-range list, marks every covered frame's bit `1` (Reserved), copies the list into the cached `reserved_ranges` array (remaining slots `None`), and sets `free_count = total - reserved_count`. Sets `hint` to the first frame *not* in any reserved range. | Bitmap reflects "Reserved at every reserved-region frame, Free elsewhere"; `hint = first_free_frame`; `free_count` = correct; `reserved_ranges` = caller's list | The PMM is now ready to accept `alloc_frame`. The reservation discipline is one-shot at boot; no later call moves a Reserved frame to Free. The fail-fast validations make the row-1 `from_aligned` unwrap **structurally** (not by-BSP-discipline) provably-correct: `extent.start` page-aligned + `i × 4096` preserve-aligned. |
...
| 2 | Bit `i` = 1 (Allocated); `hint = i + 1`; `free_count` reduced | `free_frame(PhysFrame(...))` validates the frame's PA falls within the managed `extent` (`pa >= extent.start && pa < extent.end`) — returns `Err(PmmError::OutOfRange)` if not, **before** any index arithmetic (so that an attacker / misbehaving caller passing an unrelated PhysFrame — e.g., a kernel-static buffer's PA, an MMIO base, or a frame from a different PMM instance — cannot underflow `pa - extent.start` or overflow into invalid bitmap bytes); computes `i` from the validated PA (`(pa - extent.start) / 4096`); scans the populated `Some(_)` slots of `reserved_ranges` — if `pa` falls in any reserved range, returns `Err(PmmError::DoubleFree)` without mutation (the defensive scan from the Critical-row note below); reads the bit — if already `0` (already Free) returns `Err(PmmError::DoubleFree)`; clears bit `i`; if `i < hint`, sets `hint = i` (rewind); increments `free_count` | Bit `i` = 0 (Free); `hint = min(prev_hint, i)`; `free_count` incremented | Frame is reclaimable; the rewind discipline keeps the hint pointing at the lowest known free frame, preventing fragmentation from accumulating linearly with alloc / free interleaving. **Critical row:** the bitmap collapses Reserved + Allocated into a single "1" bit. A `free_frame` of a Reserved frame *cannot* be detected by the bitmap alone (the bit is `1` either way) — but it cannot occur in well-formed kernel code: no caller ever holds a `PhysFrame` whose PA falls in a reserved range (reservation is one-way at init; reserved-frame `PhysFrame` values are never minted by the PMM). The kernel-discipline contract is that `free_frame` is only called with frames returned from `alloc_frame`. A misbehaving caller passing a Reserved-frame PA would corrupt the bitmap by clearing a Reserved bit — handled by **defensive validation**: `free_frame` cross-checks that the index `i` is *not* in any reserved range (PMM caches the reserved-range list for this check) and returns `Err(PmmError::DoubleFree)` if it is. The check is O(number of reserved ranges) — small constant for v1's BSP (3 ranges); T-017 formalises this as iterating only the `Some(_)` slots of the cached `[Option<PhysFrameRange>; R]` array (where `R` is T-017's per-BSP const generic for the array capacity, distinct from the iteration cost). The bitmap itself stays single-bit. |
| 3 | Bitmap fully `1`; `free_count = 0`; `hint = N` | `alloc_frame()` scans from `hint`, wraps to start (the hint-rewind discipline makes the wrap a separate scan from the initial `hint..N` pass), scans `0..hint` looking for any `0` bit, finds none | Bitmap unchanged; `hint` unchanged; `free_count = 0` | Returns `None`; caller propagates as `MmuError::OutOfFrames` per the `FrameProvider` contract. The forward-from-hint scan is sufficient for v1 correctness under the single-core cooperative model — the unconditional rewind in row 2 keeps `hint ≤ lowest-free-index`, so any free frame is reachable on the forward pass. The wrap-then-scan-`0..hint` step is **forward-compat scaffolding** for the future SMP extension where per-core caches may leave the global hint stale (a peer core's free without rewinding *this* core's hint). v1 never executes the wrap because the precondition `hint > lowest_free` cannot occur. |
...
5. **No new audit-log entry expected; Amendment-vs-new-entry call
   adjudication-deferred to T-017's security review.** The frame-
   zeroing step is the only new memory-touching operation; the ADR
   commits to extending UNSAFE-2026-0001's umbrella via a new
   Amendment naming the PMM site. The call is borderline because
   UNSAFE-2026-0001's original scope (PL011 MMIO base blessing —
   "this PA is a typed MMIO window the kernel exclusively owns")
   is semantically distant from PMM frame-zeroing (writing
   normal-cached RAM via raw pointer with the bitmap as the
   exclusive-ownership proof). T-017's security-review pass
   adjudicates whether the umbrella holds or whether a new entry
   (UNSAFE-2026-NNNN) is warranted. If the new-entry verdict
   lands, ADR-0035's "zero new audit entries" §Positive
   consequence flips to "one new audit entry" and the §Decision
   drivers framing is amended in T-017's commit body — not a
   re-review-of-ADR-0035 trigger. — T-017
...
- **Capability-system-ready extension.** Future `MemoryRegionCap` grants sit on top of `Pmm::alloc_frame`; the PMM stays unaware of capability identity. The reservation-list API already supports "pre-allocate N frames and hand them to layer X" without the PMM caring about X's nature. The runtime-kernel-internal framing (BSP touches the PMM only at construction time; runtime callers are kernel-only) is the v1 boundary; the capability layer wraps `alloc_frame` with a cap-check before the userspace boundary opens.
...
- Pro: regime-independent **metadata addressing** (high-half-portable; the future high-half ADR-0033 relocates the bitmap base address without algorithm change). The frame-zeroing step itself remains identity-mapping-dependent in v1 and will need a `phys_to_virt` helper post-high-half — but that helper is needed for any allocator that touches frame contents (including the rejected Option C, which additionally needs it for *every* alloc/free), so the regime-independence advantage over Option C still stands at the algorithm-and-metadata level.
- Pro: trivial reservation tracking (init walks the list, sets bits).
- Pro: zero new `unsafe` audit entries (Amendment-vs-new-entry call deferred to T-017's security review per §Dependency chain step 5; either outcome bounded).
...
- Con: 128 KiB metadata for 128 MiB with `u32` indices (64 KiB with `u16`, but `u32` is what's forward-portable to BSPs with > 64 K frames) — still 16–32× the bitmap. Out of proportion to the v1 win.
