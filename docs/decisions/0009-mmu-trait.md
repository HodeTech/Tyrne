...
- **2026-05-31 — `translate` read-only walk query added by [ADR-0038](0038-mmu-translate-and-user-access.md) / T-025.** The "Translation walk queries" item catalogued under §Open questions above (*"`lookup(va) -> Option<(PhysFrame, MappingFlags)>` … deferred until a concrete caller needs it"*) gets its concrete caller in B6 gate #1: the syscall copy-user path must resolve a user VA through the task's own `TTBR0_EL1` and confirm the leaf is `USER`-accessible before dereferencing it (the confused-deputy defence). ADR-0038 §Decision-outcome adds, as the realised form of that deferred query, `fn translate(&self, as_: &Self::AddressSpace, va: VirtAddr) -> Result<(PhysFrame, MappingFlags), MmuError>` (named `translate`, not the placeholder `lookup` above, per ARM terminology — "address translation" / the `AT` instruction denote the full page-table walk, whereas "lookup" connotes a TLB-only probe; the v1 query is a full walk) — a **read-only** walk (no `FrameProvider`, no mutation) returning the page frame backing `va` plus the leaf `MappingFlags`; `MmuError::NotMapped` for an absent leaf and the **existing** `MmuError::BlockMapped` (added by [ADR-0028](0028-address-space-data-structure.md) / T-018 for the `unmap` block path — **reused here, not newly added**) for a block-descriptor hit (v1 serves only 4 KiB page leaves). The aarch64 impl reuses the existing `QemuVirtMmu` L0→L3 walker (UNSAFE-2026-0025, extended for the read-only path) plus one new pure decoder `vmsav8::descriptor_bits_to_flags` (the inverse of `flags_to_descriptor_bits`, lock-shut). **Additive in the [ADR-0017](0017-ipc-primitive-set.md) sense:** the existing `create_address_space` / `address_space_root` / `activate` / `map` / `unmap` / `invalidate_tlb_*` methods stay byte-stable; the v1 surface commitments hold. Recorded here because the trait is the canonical reference, mirroring the `MapperFlush` rider below.
