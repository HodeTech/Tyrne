...
- **Boot-time MMU activation.** ~~Currently the kernel runs with whatever translation state QEMU provides (typically MMU-off). Turning it on is a future ADR; the linker script may need adjustments for the mapped-vs-identity split.~~ *Resolved 2026-05-08 by [ADR-0027 (Kernel virtual memory layout)](0027-kernel-virtual-memory-layout.md):* B2 activates the MMU with identity-mapping (kernel in `TTBR0_EL1`; `TTBR1_EL1` reserved for future high-half ADR-0033 placeholder when B5 surfaces per-task `TTBR0_EL1` swap). The linker script gains a new `.boot_pt` section reservation for four bootstrap page-table frames (16 KiB) bracketed by `__boot_pt_*` linker symbols and placed inside the existing `.bss` range so the BSS-zero loop pre-zeros them; **the kernel image's load address stays at `0x4008_0000` and is identity-mapped** — no mapped-vs-identity split in v1. Implemented by [T-016](../analysis/tasks/phase-b/T-016-mmu-activation.md). Companion architecture chapter: [`docs/architecture/memory-management.md`](../architecture/memory-management.md).
