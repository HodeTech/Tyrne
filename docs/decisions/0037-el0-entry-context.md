...
- **2026-05-31 — review-round findings folded in (pre-implementation-commit).** An ADR + T-023 review surfaced one HIGH, three Medium, and four nit findings against the §Decision outcome / §Simulation as originally Accepted. The decision (Option 1) is intact; the body above is left as-Accepted per [ADR-0025 §Rule 2](0025-adr-governance-amendments.md), and these riders are the canonical corrections/additions:
  - **HIGH — `enter_el0` must scrub the register file before `ERET`.** The §Decision-outcome `enter_el0` sequence (set `SP_EL0`/`ELR_EL1`/`SPSR_EL1` → `ERET`) is **extended**: because the cooperative `context_switch_asm` restores only the *callee-saved* set, the caller-saved GPRs (`x0`/`x1` = context pointers, `x8` = kernel stack, `x2`–`x7`/`x9`–`x18` = scratch), `x30` (= `enter_el0`, a kernel code address), and the caller-saved SIMD registers would otherwise carry **kernel state into EL0** — a kernel-pointer/state disclosure *orthogonal to* (and uncovered by) ADR-0033's `AP=0b00`/`UXN=1` *memory* isolation, and symmetric to the gate-#1 channel the project marks security-critical. `enter_el0` now zeroes `x0`–`x30` and `v0`–`v31` (after consuming `x19`/`x20`) before `ERET`, giving EL0 a clean, defined register file. Per [CLAUDE.md #1](../../CLAUDE.md) (no ambient channel; conservative). Recorded in [UNSAFE-2026-0032](../audits/unsafe-log.md) as a load-bearing invariant; the FP-trap alternative was rejected (global `CPACR`, unhandled-trap path) in favour of the self-contained SIMD scrub.
  - **MEDIUM — the `SPSR_EL1` write is the register form, not an immediate `MSR`.** The body's shorthand `MSR SPSR_EL1, #0x3C0` denotes the *value* `0x3C0` (EL0t + `DAIF` masked); the **instruction** is `MOV x8, #0x3C0; MSR SPSR_EL1, x8` (a GPR), because the immediate `MSR` form is valid only for PSTATE fields (`DAIFSet`/`DAIFClr`/`SPSel`/…), not a named system register. The implementation and UNSAFE-2026-0032 use the register form.
  - **MEDIUM — gate #2's "should assert" is reconciled.** ADR closes gate #2 *by construction* (`SP_EL1` = the context's restored `sp`), which removes the failure mode a separate slot would carry. The phase-plan's "should assert it" is honoured as a cheap belt-and-braces: `add_user_task` `debug_assert!`s `kernel_stack_top` is non-null + 16-byte aligned.
  - **MEDIUM — the EL0-task kernel-stack size contract is stronger than `init_context`'s.** A userspace task's `kernel_stack_top` (its `SP_EL1`) must hold the ~272-byte syscall/exception trap frame + the kernel handler call tree on **every** `+0x400` trap — not merely the cooperative-switch frame. `ContextSwitch::init_user_context`'s `# Safety` now states this explicitly.
  - **MEDIUM — test-scope claim narrowed.** `tyrne-bsp-qemu-virt` is bare-metal and outside the host-test workspace, so the host test pins the **scheduler's route** through `init_user_context` (via the `test-hal` fake), **not** the real `QemuVirtCpu` slot write. The real `x19`/`x20`/`lr`/`sp` write + the scrub are verified by code/asm audit + `kernel-build`; the runtime `ERET`-into-EL0 proof is the B6 wire-up smoke (the §Simulation rows-1–3 staging).
  - **Nits.** §Context / §Dependency-chain references to "ADR-0033 §Dependency chain note" target the [#dependency-chain](0033-kernel-high-half-migration.md#dependency-chain) anchor; the §Context "the only SP setup is the one boot-time `mov sp`" should be read as "no `SP_EL1` init *dedicated to the EL0-entry scenario*" (the cooperative `context_switch_asm` also sets per-task `sp`, which is exactly what this design reuses); the `user_sp: usize` vs `kernel_stack_top: *mut u8` type asymmetry is deliberate — `user_sp` is an opaque EL0 VA the kernel never dereferences, `kernel_stack_top` is a real kernel pointer.

- **2026-05-31 — second review-round (against the committed implementation `bbc42e8`): Approve-with-nits, 2 Low + 4 nits folded in.**
  - **Low — the register scrub omitted the FP-control/status + EL0 thread-ID classes.** `enter_el0`'s scrub covered `x0`–`x30` + `v0`–`v31` but **not** `FPCR`/`FPSR`/`TPIDR_EL0`/`TPIDRRO_EL0` — all EL0-readable, with **architecturally-UNKNOWN reset values on real hardware** (the Pi 4 target), so "clean, defined register file" was over-claimed off-QEMU: `FPCR` controls EL0 rounding / flush-to-zero / default-NaN, and a userspace task that does FP without first writing `FPCR` would inherit boot's value (Linux zeroes `FPCR`/`FPSR` on `exec` for exactly this reason). The scrub now also zeroes those four. No current secret leak (the kernel does no FP arithmetic and never writes `TPIDR`), but the EL0 *environment* is now genuinely defined on real HW, not just QEMU.
  - **Low — the implicit `x19`/`x20`/`lr`/`sp` hand-off was pinned only by `size_of == 168`.** A layout-preserving field reorder (e.g. swapping `fp`/`lr`) would silently corrupt **both** `context_switch_asm` and the `enter_el0` hand-off, caught by neither the size assert nor the host tests. Added `const _: () = assert!(offset_of!(…) == …)` for all five offsets (`0/80/88/96/104`) the asm depends on — hardening the pre-existing `context_switch_asm` coupling too.
  - **Nits.** `add_user_task` now also `debug_assert!`s `user_sp` 16-byte alignment (it becomes `SP_EL0`; `SCTLR_EL1.SA0` faults a misaligned EL0 stack); the caller-facing `# Safety` (`init_user_context` + `add_user_task`) now states the **AS-must-be-active** obligation (`TTBR0_EL1` installed + `EPD0` cleared at first dispatch) that previously lived only on `enter_el0`'s doc; the scrub is 63 register-zeros + 4 special-register writes (not "67 instructions").
  - **Process note (honesty, re: the rider above).** The 2026-05-31 review-round-1 rider was authored mid-implementation and **shipped with the implementation commit `bbc42e8`, *after* this ADR's Accept (`546febc`)** — i.e. the Accept's careful-re-read ([write-adr §10](../../.agents/skills/write-adr/SKILL.md)) missed the register-scrub HIGH; it was caught by the review, not before Accept. Recorded here rather than hidden, per [ADR-0025 §Rule 2](0025-adr-governance-amendments.md) ("riders are how implementation feedback enters the design history") — the signal is the *rate* of post-Accept riders, noted for the B6 retro.
